...
// Country mapping — raw `state` values mix US states, countries, oceans, regions,
// abbreviations, and misspellings; map known values to a canonical country (null otherwise)
...
const browseIndex = cases.map(c => ({
  ...
  location: c.location,
  state: c.state,
  country: countryForState(c.state) || '',
...
// Distinct countries (mapping defined above the browse index)
const countryCounts = {};
for (const c of cases) {
